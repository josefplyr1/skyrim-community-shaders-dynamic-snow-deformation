...
public:
	...
	struct alignas(16) ShellCB
	{
		...
		float4 FineWindow;
		/** @brief x = Settings::SlopeDrape, y = normal.z at full tilt (cos 60), z = normal.z where the tilt starts (cos 30), w = spare. */
		float4 SlopeDrape;
	...
	/** @brief Top of the enclosed-shelter band (world units, fades over the next 100): a tent ridge is inside it. */
	static constexpr float kShelterBandHeight = 250.0f;
	...
	struct alignas(16) HeightProcessCB
	{
		...
		/** @brief Top of the enclosure band (units), >= ShelterMaxHeight; read only while ShelterConeTan > 0. */
		...
		/** @brief tan(ShelterDriftAngle); 0 = no enclosure band. */
	...
	struct VoxelLevel
	{
		...
		/** @brief R32_UINT, (4 dim)^2, world-anchored like the volume: the topmost captured surface over each sub-pixel as the bits of (world z + 32768) with the surface's up-ness in the low four bits, kept by an atomic max, so a column's height is deterministic and prefiltered over sixteen samples. The Z sweep prefers it; `height` is the fallback for a seed under an overhang. */
	...
	/** @brief Where a level's bricks and draw end (kVoxelReachFrac) and where its hand-over band starts (kVoxelBandFrac before that), Chebyshev units from the camera. */
	void VoxelReachBand(uint a_level, float& a_start, float& a_end) const;
