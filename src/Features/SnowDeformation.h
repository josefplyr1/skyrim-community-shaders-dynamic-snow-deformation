...
public:
	...
	struct Settings
	{
		...
		/** @brief "Volume Edge Rounding", world units, 0..16: the sideways smoothing among snow and the radius the lip's end rolls off over. Never shortens the lip (Overhang alone does); 0 is blocky. Josef's tuned default. */
	...
	struct alignas(16) VoxelVolumeCB
	{
		...
		/** @brief Sideways smoothing sigma in voxels among snow, from Settings::VolumeSnowRounding: a voxel at least, 0.25 when the Rounding is 0. */
		float RoundSigma;
		...
		float ForceDirty;
		/** @brief Settings::VolumeSnowOverhang in this level's voxels, rounded, 0..15: how far an air voxel copies the nearest snow, and where the lip is cut. */
		float OverhangVox;
		...
		float EdgeParams[4];
		/** @brief x = the Rounding in this level's voxels, unfloored: the lip's roll-off radius (air no longer votes in the sideways average). y = the capture's conservative expansion in voxels (Settings::VolumeConservativeCapture ? 0.5 : 0). */
		float LipParams[4];
