...
void SnowDeformation::DrawSettings()
{
	...
	if (ImGui::TreeNodeEx(T(TKEY("model_depths"), "Snow Depth by Model Class"), ImGuiTreeNodeFlags_Framed)) {
		...
		if (auto _ttEdgeR = Util::HoverTooltipWrapper())
			ImGui::Text("%s", T(TKEY("edge_lump_reach_tooltip"), "How far past the edge of the painted snow the lumps hang on, onto bare rock: up to about half a metre at 1, none at 0. The band keeps the snow's own ragged edge next to the solid part and breaks into round lumps toward its end. Only real edges count: a face frosted faintly all over has no edge and stays clean. Needs Recolor Projected Snow, and only objects that carry the game's own projected-snow data take part."));
