...
void SnowDeformation::DrawSettings()
{
	...
	if (ImGui::TreeNodeEx(T(TKEY("model_depths"), "Snow Depth by Model Class"), ImGuiTreeNodeFlags_Framed)) {
		...
		if (auto _tt3d = Util::HoverTooltipWrapper())
			ImGui::Text("%s", T(TKEY("object_snow_3d_tooltip"), "Our own snow SURFACE on objects: a rounded blanket grown over the Snow Fill area where the game paints projected snow - its edge rolls over like a real snow lip, and the rounding lengthens as the depth rises. Off, that surface is not drawn at all - not the rise, not the flat coat, none of it, since the coat is the same layer with no height rather than a separate thing. What the object keeps is its OWN painted snow, recoloured to match the ground shell while Recolor Projected Snow is on, so the two still agree in colour and only the shape is gone. Objects without projected-snow data carry no layer either way; roads and their trenches are separate machinery and stay on; and objects go on lifting and sheltering the ground shell around them however this is set."));
