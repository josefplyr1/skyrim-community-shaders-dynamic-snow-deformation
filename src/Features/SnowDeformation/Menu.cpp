...
void SnowDeformation::DrawSettings()
{
	...
	if (ImGui::TreeNodeEx(T(TKEY("model_depths"), "Snow Depth by Model Class"), ImGuiTreeNodeFlags_Framed)) {
		...
		if (auto _ttEdgeR = Util::HoverTooltipWrapper())
			ImGui::Text("%s", T(TKEY("edge_lump_reach_tooltip"), "How far past the solid snow's edge the lumps hang on, up to about half a metre at 1; 0 draws no lumps at all. The band keeps the snow's own ragged edge next to the solid part and breaks into round lumps toward its end. Only real edges count: a face the game frosts faintly all over, like a house wall, has no edge and stays clean."));
