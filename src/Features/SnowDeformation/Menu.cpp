...
void SnowDeformation::DrawSettings()
{
	...
	if (ImGui::TreeNodeEx(T(TKEY("model_depths"), "Snow Depth by Model Class"), ImGuiTreeNodeFlags_Framed)) {
		...
		if (auto _tt3d = Util::HoverTooltipWrapper())
			ImGui::Text("%s", T(TKEY("object_snow_3d_tooltip"), "The RAISED 3D snow layer on objects: a rounded blanket grown over the Snow Fill area where the game paints projected snow - its edge rolls over like a real snow lip, and the rounding lengthens as the depth rises. Off, the shape goes entirely - no rise, no rolled edge, no edge lumps - and what stays is the flat drape that repaints the object's own projected snow in the shell's material, which belongs to Recolor Projected Snow above. Turn BOTH off and nothing of ours is drawn on an object at all. Objects without projected-snow data carry no layer either way; roads and their trenches are separate machinery and stay on; and objects go on lifting and sheltering the ground shell around them however this is set."));
