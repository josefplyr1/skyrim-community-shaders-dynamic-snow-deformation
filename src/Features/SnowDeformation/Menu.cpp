...
void SnowDeformation::DrawSettings()
{
	...
	if (ImGui::TreeNodeEx(T(TKEY("debug_options"), "Debugging Options"), ImGuiTreeNodeFlags_Framed)) {
		...
		ImGui::Checkbox(T(TKEY("statics_record_enabled"), "Object Snow: Per-Draw Constant Offsets (D3D11.1)"), &staticsRecordEnabled);
		if (auto _ttRecord = Util::HoverTooltipWrapper())
			ImGui::Text("%s", T(TKEY("statics_record_enabled_tooltip"), "Experimental, OFF by default. Uploads every object's constants once per frame and binds each draw's block by offset instead of updating a constant buffer per draw (about 3,500 updates a frame). Needs a real Direct3D 11.1 context: a DLSS or frame-generation interposer can hand back a fake one and the driver crashes on the first draw. Turn on only with a save you can lose; the log says what the query returned."));
