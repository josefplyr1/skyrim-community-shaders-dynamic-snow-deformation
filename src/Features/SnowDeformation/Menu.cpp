	...
	if (ImGui::TreeNodeEx(T(TKEY("model_depths"), "Object Snow"), ImGuiTreeNodeFlags_Framed)) {
		...
		if (settings.VolumeSnow) {
			...
			if (auto _ttVoxRound = Util::HoverTooltipWrapper())
				ImGui::Text("%s", T(TKEY("volume_snow_rounding_tooltip"), "How round the snow is, in units: the lip's end rolls off over this much of its length, and steps between columns blend over it. It never shortens the lip - Overhang alone sets the reach. 0 is blocky. On a ring whose voxels are larger than this the far snow is smoothed a voxel and keeps its slab."));
