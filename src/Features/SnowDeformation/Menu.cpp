...
void SnowDeformation::DrawSettings()
{
	...
	if (ImGui::TreeNodeEx(T(TKEY("model_depths"), "Object Snow"), ImGuiTreeNodeFlags_Framed)) {
		...
		ImGui::Checkbox(T(TKEY("edge_reach_by_weight"), "Edge Reach By Weight"), &settings.EdgeReachByWeight);
		if (auto _ttEdgeRw = Util::HoverTooltipWrapper())
			ImGui::Text("%s", T(TKEY("edge_reach_by_weight_tooltip"), "A/B, wins over Edge Reach On Surface. No disc at all: the game's own projected-snow weight is read at each pixel, and Edge Lump Reach lowers the level at which it counts as snow - by half a unit of weight at 1. At 0 it is exactly the game's paint; raising it lets the snow creep down along the game's own falloff (slope, painted alpha, normal map and noise included). The same from every angle. Known limit: a face whose weight is the same all over has nowhere to creep from and flips at once when the level reaches it. Needs a ShaderCache clear after the update that added it."));
		ImGui::Checkbox(T(TKEY("edge_reach_on_surface"), "Edge Reach On Surface"), &settings.EdgeReachOnSurface);
		if (auto _ttEdgeRs = Util::HoverTooltipWrapper())
			ImGui::Text("%s", T(TKEY("edge_reach_on_surface_tooltip"), "A/B. The reach disc is laid flat on the object's own surface in world units and scored from the projected snow's rebuilt weight there, instead of from the pixels around this one on screen. The spread should then hold still as the camera moves. Known limits: it follows the rebuilt weight rather than the game's painted pixels, and on small or thin shapes the reach shortens to the part of the surface that has not turned away. The Reach compare debug view (Object Snow, Debug View) shows both at once: white = the game's solid paint, red = only the screen disc spreads here, green = only this one, yellow = both."));
	...
	if (ImGui::TreeNodeEx(T(TKEY("debug_options"), "Debugging Options"), ImGuiTreeNodeFlags_Framed)) {
		...
		if (ImGui::TreeNodeEx(T(TKEY("debug_cat_object_snow"), "Object Snow"))) {
			{
				const char* staticsDebugModes[] = { "Off", "Edge taper", "Coverage alpha", "Normals", "Self-shadow march", "Projected mask", "Shell layers", "Lift gradient", "Skin identity", "Depth fight", "Volume seed", "Reach compare", "Reach readings" };
