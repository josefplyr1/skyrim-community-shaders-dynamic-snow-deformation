...
void SnowDeformation::DrawSettings()
{
	...
	if (ImGui::TreeNodeEx(T(TKEY("debug_options"), "Debugging Options"), ImGuiTreeNodeFlags_Framed)) {
		...
		if (debugActorContact)
			ImGui::Text("  actors: %u living + %u corpses rasterized (caps %u / %u), %u partition draws (bone stamps skipped for these), %u missing-bone stand-ins",
				stampStats.actorsRasterized, stampStats.corpsesRasterized, kContactMaxActors, kContactMaxCorpses, contactSkinDrawsLast, contactSkinMissingLast);
