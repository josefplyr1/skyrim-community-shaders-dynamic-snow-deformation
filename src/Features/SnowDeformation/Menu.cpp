...
void SnowDeformation::DrawSettings()
{
	...
	if (ImGui::TreeNodeEx(T(TKEY("model_depths"), "Snow Depth by Model Class"), ImGuiTreeNodeFlags_Framed)) {
		...
		if (auto _tt3d = Util::HoverTooltipWrapper())
			ImGui::Text("%s", T(TKEY("object_snow_3d_tooltip"), "The 3D snow layer on objects: a rounded blanket grown over the Snow Fill area where the game paints projected snow - its edge rolls over like a real snow lip, and the rounding lengthens as the depth rises. Off, there is NO layer on objects at all: nothing of ours is drawn over them. What the object keeps is its own painted snow, recoloured to match the ground shell if Recolor Projected Snow is on - so the snow still agrees, it just has no shape. Objects without projected-snow data carry no layer either way. Roads and their trenches are separate machinery and stay on, and objects still lift and shelter the ground shell around them."));
