...
void SnowDeformation::DrawSettings()
{
	...
	if (ImGui::TreeNodeEx(T(TKEY("debug_options"), "Debugging Options"), ImGuiTreeNodeFlags_Framed)) {
		...
		if (auto _ttRecord = Util::HoverTooltipWrapper())
			ImGui::Text("%s", T(TKEY("statics_record_enabled_tooltip"), "Uploads every object's constants once per frame and binds each draw's block by offset instead of updating a constant buffer per draw (about 3,500 updates a frame): the same pixels for less CPU. Needs Direct3D 11.1 constant-buffer offsetting; where the driver or an interposer says no, the log records why and every draw takes the old path. Measurement aid: untick to A/B."));
