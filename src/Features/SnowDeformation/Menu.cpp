...
void SnowDeformation::DrawSettings()
{
	...
	if (ImGui::TreeNodeEx(T(TKEY("model_depths"), "Object Snow"), ImGuiTreeNodeFlags_Framed)) {
		...
		ImGui::Checkbox(T(TKEY("ice_floe_snow"), "Recolor PD on Ice Floes"), &settings.IceFloeSnow);
		if (auto _ttFloe = Util::HoverTooltipWrapper())
			ImGui::Text("%s", T(TKEY("ice_floe_snow_tooltip"), "PD = projected diffuse, the snow the game paints onto objects. On, ice floes (any mesh with 'floe' in its name) have that painted snow recolored to this mod's snow and get the drape and volume snow like every other object. Off, the ice floes keep the game's own painted snow untouched, with no drape and no volume snow. The ice itself is never recolored either way."));
