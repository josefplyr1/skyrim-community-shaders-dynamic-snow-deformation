	...
	if (ImGui::TreeNodeEx(T(TKEY("model_depths"), "Object Snow"), ImGuiTreeNodeFlags_Framed)) {
		...
		if (settings.VolumeSnow) {
			...
			if (auto _ttVoxOverhang = Util::HoverTooltipWrapper())
				ImGui::Text("%s", T(TKEY("volume_snow_overhang_tooltip"), "How far past an edge the snow extends sideways, in units, whatever its depth or Rounding - the lip is cut here. Also how readily the snow of two nearby surfaces melds into one. Solid still stops it: snow never spreads through a wall or up a step riser, so this is the reach across open air only."));
			ImGui::SliderFloat(T(TKEY("volume_snow_rounding"), "Rounding"), &settings.VolumeSnowRounding, 0.0f, 16.0f, "%.0f u");
			if (auto _ttVoxRound = Util::HoverTooltipWrapper())
				ImGui::Text("%s", T(TKEY("volume_snow_rounding_tooltip"), "The corner radius of the snow's cross-section, in units: how round its top edges and a lip's underside are, up to half the depth. It never moves the flat top or shortens the lip - Overhang alone sets the reach. 0 is square. Steps between neighbouring columns blend over about this distance as well."));
