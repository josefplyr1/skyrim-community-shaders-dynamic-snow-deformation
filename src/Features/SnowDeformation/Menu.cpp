...
void SnowDeformation::DrawSettings()
{
	...
	if (ImGui::TreeNodeEx(T(TKEY("undulation"), "Surface Undulation"), ImGuiTreeNodeFlags_Framed)) {
		...
		if (auto _ttPs2 = Util::HoverTooltipWrapper())
			ImGui::Text("%s", T(TKEY("parallax_steps_tooltip"), "Coarse steps in the parallax march, before contact refinement re-marches the hit interval at the same budget (so 8 resolves roughly like 64). Scaled down with distance and off entirely past the band where the snow grain stops being drawn. The main quality-versus-cost dial: each step is a 3-tap blend, so raising it is the most expensive thing in this panel."));
