...
void SampleBloodTiles(float2 worldXY, float2 worldDx, float2 worldDy, inout float4 blood, inout float fresh, out float4 paint)
{
	paint = 0.0;
	// 2 = direct decals: SampleBloodDecals reads the tiles instead.
	[branch] if (BloodLook2.w > 0.5 && BloodLook2.w < 1.5)
	{
		...
		[branch] if (slot1 != 0u)
		{
			...
			const int2 texel = clamp((int2)local, 0, kBloodTileDim - 1);
...
#if defined(PSHADER) && defined(SNOW_RUNE_GLYPHS)
// Rune glyphs (Blood.cpp, RenderRuneCapture), for the shells that define
// SNOW_RUNE_GLYPHS - the landscape shell and the road patch; object skins
// get theirs from the decal overlay. A 2x2 atlas, one tile per live
...
#	if defined(SNOW_BLOOD_TILES)
// Blood decals painted directly (BloodDecals.cpp), the A/B partner of the
// detail tiles: a tile holds each decal's uv field and texture slot, and the
// game's own blood texture and normal map are sampled through it, as a rune's
// are. Point-read: texels of different decals lie side by side, so the uv is
// rebuilt from one texel and its same-decal neighbours, never filtered.
Texture2D<float4> BloodDecalDiffuse0 : register(t105);
Texture2D<float4> BloodDecalDiffuse1 : register(t106);
Texture2D<float4> BloodDecalDiffuse2 : register(t107);
Texture2D<float4> BloodDecalDiffuse3 : register(t108);
Texture2D<float4> BloodDecalDiffuse4 : register(t109);
Texture2D<float4> BloodDecalDiffuse5 : register(t110);
Texture2D<float4> BloodDecalDiffuse6 : register(t111);
Texture2D<float4> BloodDecalDiffuse7 : register(t112);
Texture2D<float4> BloodDecalDiffuse8 : register(t113);
Texture2D<float4> BloodDecalDiffuse9 : register(t114);
Texture2D<float4> BloodDecalDiffuse10 : register(t115);
Texture2D<float4> BloodDecalNormal0 : register(t116);
Texture2D<float4> BloodDecalNormal1 : register(t117);
Texture2D<float4> BloodDecalNormal2 : register(t118);
Texture2D<float4> BloodDecalNormal3 : register(t119);
Texture2D<float4> BloodDecalNormal4 : register(t120);
Texture2D<float4> BloodDecalNormal5 : register(t121);
Texture2D<float4> BloodDecalNormal6 : register(t122);
Texture2D<float4> BloodDecalNormal7 : register(t123);
Texture2D<float4> BloodDecalNormal8 : register(t124);
Texture2D<float4> BloodDecalNormal9 : register(t125);
Texture2D<float4> BloodDecalNormal10 : register(t126);

RuneGlyph SampleBloodDecals(float2 worldXY, float2 worldDx, float2 worldDy)
{
	RuneGlyph g;
	g.colour = 0.0;
	g.normalTS = float3(0.0, 0.0, 1.0);
	...
	g.bitangent = float3(0.0, 1.0, 0.0);
	g.emission = 0.0;
	[branch] if (BloodLook2.w > 1.5)
	{
		const float cellSize = BloodLook3.z;
		const float texelWorld = BloodLook3.w;
		const float apron = 0.5 * (float(kBloodTileDim) * texelWorld - cellSize);
		const int2 cell = (int2)floor(worldXY / cellSize);
		const uint slot1 = BloodTileIndex.Load(int3(cell & 63, 0));
		[branch] if (slot1 != 0u)
		{
			const uint tileSlot = slot1 - 1u;
			const int2 tileTexel = int2(tileSlot % 4u, tileSlot / 4u) * kBloodTileDim;
			const float2 local = (worldXY - (float2(cell) * cellSize - apron)) / texelWorld;
			const int2 texel = clamp((int2)local, 1, kBloodTileDim - 2);
			const float4 c = BloodTilePigment.Load(int3(tileTexel + texel, 0));
			[branch] if (c.b > 0.0)
			{
				// A neighbour of the same decal on either side gives the
				// field's slope; none gives a flat texel.
				const float4 xp = BloodTilePigment.Load(int3(tileTexel + texel + int2(1, 0), 0));
				const float4 xn = BloodTilePigment.Load(int3(tileTexel + texel - int2(1, 0), 0));
				const float4 yp = BloodTilePigment.Load(int3(tileTexel + texel + int2(0, 1), 0));
				const float4 yn = BloodTilePigment.Load(int3(tileTexel + texel - int2(0, 1), 0));
				const float same = 0.5 / 255.0;
				float2 uvDx = abs(xp.b - c.b) < same ? xp.rg - c.rg : (abs(xn.b - c.b) < same ? c.rg - xn.rg : 0.0);
				float2 uvDy = abs(yp.b - c.b) < same ? yp.rg - c.rg : (abs(yn.b - c.b) < same ? c.rg - yn.rg : 0.0);
				uvDx /= texelWorld;
				uvDy /= texelWorld;
				const float2 off = (local - (float2(texel) + 0.5)) * texelWorld;
				const float2 uv = c.rg + uvDx * off.x + uvDy * off.y;
				const float2 gradX = uvDx * worldDx.x + uvDy * worldDx.y;
				const float2 gradY = uvDx * worldDy.x + uvDy * worldDy.y;
				float4 d = 0.0;
				float4 n = float4(0.5, 0.5, 1.0, 0.0);
				[branch] switch ((uint)round(c.b * 255.0) - 1u)
				{
				case 0:
					d = BloodDecalDiffuse0.SampleGrad(SnowSampler, uv, gradX, gradY);
					n = BloodDecalNormal0.SampleGrad(SnowSampler, uv, gradX, gradY);
					break;
				case 1:
					d = BloodDecalDiffuse1.SampleGrad(SnowSampler, uv, gradX, gradY);
					n = BloodDecalNormal1.SampleGrad(SnowSampler, uv, gradX, gradY);
					break;
				case 2:
					d = BloodDecalDiffuse2.SampleGrad(SnowSampler, uv, gradX, gradY);
					n = BloodDecalNormal2.SampleGrad(SnowSampler, uv, gradX, gradY);
					break;
				case 3:
					d = BloodDecalDiffuse3.SampleGrad(SnowSampler, uv, gradX, gradY);
					n = BloodDecalNormal3.SampleGrad(SnowSampler, uv, gradX, gradY);
					break;
				case 4:
					d = BloodDecalDiffuse4.SampleGrad(SnowSampler, uv, gradX, gradY);
					n = BloodDecalNormal4.SampleGrad(SnowSampler, uv, gradX, gradY);
					break;
				case 5:
					d = BloodDecalDiffuse5.SampleGrad(SnowSampler, uv, gradX, gradY);
					n = BloodDecalNormal5.SampleGrad(SnowSampler, uv, gradX, gradY);
					break;
				case 6:
					d = BloodDecalDiffuse6.SampleGrad(SnowSampler, uv, gradX, gradY);
					n = BloodDecalNormal6.SampleGrad(SnowSampler, uv, gradX, gradY);
					break;
				case 7:
					d = BloodDecalDiffuse7.SampleGrad(SnowSampler, uv, gradX, gradY);
					n = BloodDecalNormal7.SampleGrad(SnowSampler, uv, gradX, gradY);
					break;
				case 8:
					d = BloodDecalDiffuse8.SampleGrad(SnowSampler, uv, gradX, gradY);
					n = BloodDecalNormal8.SampleGrad(SnowSampler, uv, gradX, gradY);
					break;
				case 9:
					d = BloodDecalDiffuse9.SampleGrad(SnowSampler, uv, gradX, gradY);
					n = BloodDecalNormal9.SampleGrad(SnowSampler, uv, gradX, gradY);
					break;
				case 10:
					d = BloodDecalDiffuse10.SampleGrad(SnowSampler, uv, gradX, gradY);
					n = BloodDecalNormal10.SampleGrad(SnowSampler, uv, gradX, gradY);
					break;
				default:
					break;
				}
				g.colour = float4(d.rgb, saturate(d.a * c.a * BloodLook.x));
				const float det = uvDx.x * uvDy.y - uvDy.x * uvDx.y;
				[branch] if (n.a + n.z > 0.0 && abs(det) > 1e-12)
				{
					g.normalTS = normalize(n.xyz * 2.0 - 1.0);
					g.gloss = n.a;
					g.tangent = normalize(float3(uvDy.y, -uvDx.y, 0.0) / det);
